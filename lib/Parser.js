...
class Parser {
  ...
  parseCall(func) {
    // se context for handling brackets inside of arguments
  ...
  parseSquareBracket() {
    switch (this.currentNode.type) {
      case "call":
        return {
          type: "numericals",
          vars: this.delimited("[", "]", null, this.parseExpression.bind(this)),
        };
      default:
        return {
          type: "variables",
  ...
  maybeBinary(left, curPrec) {
    ...
    if (tok) {
      ...
      if (nextPrec > curPrec) {
        ...
        returnType = this.parseReturnType(tok.value);
