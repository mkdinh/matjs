...
class Parser {
  ...
  parseCall(func) {
    ...
    const expr = {
      type: "call",
      func: func,
      isPrimitive: this.isPrimitiveFunc(func.value),
    ...
    if (this.isOp("=") && !expr.isPrimitive) {
      expr.type = "array";
      expr.action = "index";
    } else if (this.isOp() && !expr.isPrimitive) {
      expr.type = "array";
      expr.action = "binary";
    }
  ...
  maybeArray(context, expr) {
    if (this.FUNCTIONS.indexOf(context.value) >= 0) {
      return expr();
    } else if (context && this.arrayIdentifiers.indexOf(context.value)) {
      const expr = {
        type: "call",
        primitive: true,
  ...
  maybeCall(expr) {
    expr = expr();
    return this.isPunc("(")
      ? this.maybeArray(expr, () => this.parseCall(expr))
      : expr;
  }
