...
class Parser {
  ...
  parseSquareBracket() {
    // assumed that all brackets are inline array
    let expr = {
      type: "array",
      action: "create",
      elements: this.delimited("[", "]", ",", this.parseExpression.bind(this)),
    };
    // if the next character after bracket is a '='
    // assume that the array is spreaded
    if (this.isOp("=")) {
  ...
  maybeArrayIndexing(expr, next) {
    // if the next character is an '=' and not a primitive, then it is an
    // indexing of array, not a func calls

    const isPrimitive = expr.primitive;
    if (this.isOp("=") && !isPrimitive) {
      expr.type = "array";
      expr.action = "index";
    } else if (this.isOp() && !isPrimitive) {
      expr.type = "array";
      expr.action = "binary";
    }

    if (expr.type === "array") {
      this.arrayIdentifiers.push(expr.func.value);
      console.log(expr);
      return expr;
    } else {
      return next();
  ...
  maybeArrayOp(binary, expr) {
    ...
    if (this.isArrayBinary(binary)) {
      return this.indexArrayTok(binary);
    }
  ...
  // Helper methods
  //--------------------------------------------------------
  indexArrayTok(binary) {
    let array, scalar, opTok;
    opTok = { type: "op", value: binary.op };
    // determine which expression is the array or the scalar
